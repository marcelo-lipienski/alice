...
class UserService {
  async all() {
    try {
      const users = await _User.default.find({}, '-password -jwt');
  ...
  async authenticate(email, password) {
    ...
    const password_match = await _bcrypt.default.compare(password, user.password);
    ...
    if (password_match) {
      ...
      return _ResponseBuilder.default.success(200, {
        token: token
      });
    } else {
      return _ResponseBuilder.default.error(400);
